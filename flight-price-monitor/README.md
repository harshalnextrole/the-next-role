...
- Searches Amadeus for business class flights across 15 date combinations (Oct–Dec 2026)
...
- Runs every 8 hours via GitHub Actions (free)
...
2. Create a **production** app (test environment uses cached data)
...
```
Every 8 hours:
  1. Generate 15 date pairs (Oct 1 – Dec 10, every 5 days)
  2. For each pair, search Amadeus for business class YYZ→DEL
  3. Filter: ≤1 stop, ≤26h, ≤6h layover
...
|---|---|
| Amadeus API (~1,350 calls/month) | $0 (free tier) |
| Resend emails (~20–30/month) | $0 (free tier) |
| GitHub Actions (~180 min/month) | $0 (free tier) |
| **Total** | **$0/month** |
